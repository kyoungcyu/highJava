...
public class T01FileTest {
	...
	System.out.println("파일 여부 : " +file.isFile()); //파일이면 true
	System.out.println("디렉토리(폴더) 여부 : "+ file.isDirectory());//폴더가 아니라서 false
	System.out.println("-------------------------------------------");
	...
	File file4 = new File(".\\D_Other\\test\\..", "test.txt"); //경로
	...
	/*
	  ...
	  1. mkdir()  => File객체의 경로 중 마지막 위치의 디렉토리를 만든다.
	  			   	 중간의 경로가 모두 만들어져 있어야한다.
	  			 	
	  2. mkdirs() => 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 
	 				 만들어 준다.
	 
	 => 위 두 메소드 모두 만들기를 성공하면 true, 실패하면 false 반환
	 */
	...
	if(file5.mkdir()) {
		...
		System.out.println(file5.getName() + "만들기 실패 ㅠ!");
	}
	System.out.println();
	...
	if(file6.mkdirs()) {
		...
		System.out.println(file6.getName() + "만들기 실패 ㅠ!");
	}
	System.out.println();
