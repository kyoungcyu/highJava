...
/*
    ...
    ReentrantLock : Read 및 Write 구분없이 사용하기 위한 락 클래스로 동시화 처리를 위해 사용된다.
    ...
    ReentrantReadWriteLock : Read 및 Write 락을 구분하여 사용가능함.
                             ...
                             => Write 보다 Read 위주의 작업이 많이 발생하는 경우에 사용하면 처리량(Throughput)이 좋아진다.
   ...
   */
   ...
   public boolean withdraw(int money) {
      ...
      try {
         ...
         lock.unlock(); // 락 해제
         System.out.println(Thread.currentThread().getName() + "락 설정(반납) 완료!");
      }
...
class BankThread2 extends Thread{
   ...
   public void run() {
      ...
      System.out.println(Thread.currentThread().getName() + "스레드 안에서 result = " + result + ", balance = " 
                     + lAcc.getBalance());
