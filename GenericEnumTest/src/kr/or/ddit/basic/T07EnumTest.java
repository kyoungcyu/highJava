...
public class T07EnumTest {
	 ...
	 * enum Data {A,B,C,D};
	 * 
	  * 열거형 선언방법 enum 열거형이름 {상수값1, 상수값2, 상수값3, ..., 상수값n};
	 */
	// City 열거형 정의 (기본값을 이용하는 열거형)
	public enum City {서울(), 부산(), 대구(), 광주(), 대전()};
//	public enum HomeTown {서울, 부산, 대구,광주, 대전};         안됨 

	// 데이터값을 임의로 지정한 열거형 객체 선언
	// 데이터값을 정해 줄 경우에는 생성자를 만들어서 괄호속의 값이 변수에 저장되도록 해야 한다.
	public enum Season {
		봄("3월부터 5월 까지 "), 여름("6월부터 8월 까지"), 가을("9월부터 11월까지"), 겨울("12월부터 2월까지");

		// 괄호속의 값이 저장될 변수 선언
		private String str;

		// 생성자 만들기 (열거형의 생성자는 제어자가 묵시적으로 'private' 이다 _)
	...
	/**
	 열거형에서 사용되는 메소드
	 	...
	 	*3.valueOf("열거형상수이름"); => 지정된 열거형에서 '열거형상수이름'과 일치하는 열거형 상수를
	 									반환한다.
	 */
		...
		System.out.println("myCity1의 ordinal : " + myCity1.ordinal()); //열거형 배열안에 순서 
		System.out.println();
		...
		for(int i=0; i<enumArr.length;i++) { //상수값을 가진를 문자열로 반환하고 ()
			System.out.println(enumArr[i].name()+ ":"+ enumArr[i].getStr());
		}
		
		for(City city : City.values()) { //뒤에것이 배열의 값 
